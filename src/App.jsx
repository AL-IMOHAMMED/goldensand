...
export default function App() {
  ...
  const addPurchase = async () => { if (!form.invoiceNo) return notify('❌ أدخل الرقم'); setIsLoading(true); await api.insert('purchase_invoices', { user_id: user.id, invoice_no: form.invoiceNo, supplier: form.supplier || '', date: form.date, currency: form.currency || 'USD', total: parseFloat(form.total) || 0 }, token); await loadData(); setModal(null); setForm({}); notify('✅ تم'); setIsLoading(false) }
  const addShipping = async () => { if (!form.company) return notify('❌ أدخل الشركة'); setIsLoading(true); await api.insert('shipping_costs', { user_id: user.id, company: form.company, date: form.date, currency: form.currency || 'TRY', total: parseFloat(form.total) || 0 }, token); await loadData(); setModal(null); setForm({}); notify('✅ تم'); setIsLoading(false) }
  ...
  return (
    <div dir="rtl" style={{ fontFamily: 'Arial', minHeight: '100vh', background: '#f3f4f6' }}>
      ...
      <div style={{ padding: 10 }}>
        {tab === 'home' && <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            <button onClick={() => { setForm({ date: new Date().toISOString().split('T')[0], currency: 'USD' }); setModal('purchase') }} style={{ background: '#f97316', color: '#fff', border: 'none', padding: 12, borderRadius: 8, fontWeight: 'bold', cursor: 'pointer' }}>📥 شراء</button>
        ...
        {tab === 'purchases' && <Card title="📥 فواتير الشراء" count={data.purchaseInvoices.length} onAdd={() => { setForm({ date: new Date().toISOString().split('T')[0], currency: 'USD' }); setModal('purchase') }} color="#f97316"><Table cols={['الفاتورة', 'المورد', 'المبلغ', '']} rows={data.purchaseInvoices.map(p => [p.invoice_no, p.supplier, <span key="t" style={{ color: '#f97316' }}>{Number(p.total).toLocaleString()} {p.currency}</span>, <Btn key={p.id} onClick={() => deleteItem('purchase_invoices', p.id)}>🗑️</Btn>])} /></Card>}
      ...
      {modal === 'purchase' && <Modal title="📥 فاتورة شراء" onClose={() => setModal(null)} onSave={addPurchase}><Input label="رقم الفاتورة *" value={form.invoiceNo || ''} onChange={v => setForm({ ...form, invoiceNo: v })} /><Input label="المورد" value={form.supplier || ''} onChange={v => setForm({ ...form, supplier: v })} /><Input label="التاريخ" type="date" value={form.date || ''} onChange={v => setForm({ ...form, date: v })} /><Input label="المبلغ" type="number" value={form.total || ''} onChange={v => setForm({ ...form, total: v })} /></Modal>}
      {modal === 'shipping' && <Modal title="🚚 فاتورة شحن" onClose={() => setModal(null)} onSave={addShipping}><Input label="شركة الشحن *" value={form.company || ''} onChange={v => setForm({ ...form, company: v })} /><Input label="التاريخ" type="date" value={form.date || ''} onChange={v => setForm({ ...form, date: v })} /><Input label="المبلغ" type="number" value={form.total || ''} onChange={v => setForm({ ...form, total: v })} /></Modal>}
